...
const Order = {
  ...
  getOrdersByUser: (userId, callback) => {
    db.query(
      'SELECT orders.id, products.name, products.price, orders.quantity, orders.totalPrice, orders.status FROM orders JOIN products ON orders.productId = products.id WHERE orders.userId = ?',
      [userId],
      callback
    );
  }
